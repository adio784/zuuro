    ...
    public function sendDataTransfer(Request $request){
        ...
        if($user->email_verified_at !="" && $user->isVerified == 1){
          if(Hash::check($request->pin, $user->create_pin)){
            ...
            if($request->top_up == 2 && $request->country == 'NG'){
                                ...
                                if($dataResponse['ResultCode'] ==1){
                                    ...
                                    if($query){
                                        ...
                                        $query_loan_record = DB::table('loan_record')->insert([
                                            'user_id'=> $uid,
                                            'referrence_id'=> $dataResponse['TransferRecord']['TransferId']['TransferRef'],
                                            'loan_amount'  => $dataResponse['TransferRecord']['Price']['SendValue'],
                                            'repayment'    => $dueDate,
                                            'status'       => 0
                                        ]);
                                        ...
                                        $notification = DB::table('notifications')
                                                    ->insert([
                                                        'user_id'=> $uid,
                                                        'message' => 'Successfully loan '. $dataResponse['TransferRecord']['Price']['ReceiveValue'] .'to :'. $dataResponse['TransferRecord']['AccountNumber'] .'to be paid back :'. $dueDate,
                                                    ]);
